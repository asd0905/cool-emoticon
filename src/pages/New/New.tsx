...
import {EQueryKey, IEmoticonSelector, IEmoticonSelectorData, STICKER_BASE_URL} from '../../app.constant';
        ...
        <SLayout>
            <MainBanner type={'newMainBanner'}>
                <video autoPlay={true} muted={true} loop={true} playsInline={true} >
                    <source src="//update.coolmessenger.com/_ImageServer/jirancomms/img/서비스흐름.mp4" type="video/mp4" />
                </video>
            </MainBanner>
                    ...
                    <section className={'emoticonListSect'}>
                        {emoticons.map((emoticon: any) => (
                            <article key={emoticon.name} className={'emoticonList'}>
                                ...
                                {emoticon.data.slice(0, 4).map((d: IEmoticonSelectorData) => (
                                    <li key={d.image} className={'emoticon'}>
                                        <img src={`${STICKER_BASE_URL}/${emoticon.name}/image_pack/${d.image}`} alt="img" />
                                    </li>
                                ))}
                                </ul>
                            </article>
